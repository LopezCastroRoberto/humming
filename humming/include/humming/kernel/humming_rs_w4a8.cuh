...
#include <humming/scheduler/rs_w4a8_grouped.cuh>
...
// Persistent RS W4A8 mainloop, specialized for M-major FP8 group-128 scales
// and MXFP4 group-32 weights.
#ifndef HUMMING_USE_RS_W4A8
#define HUMMING_USE_RS_W4A8 0
#endif
static_assert(HUMMING_USE_RS_W4A8,
              "humming_rs_w4a8.cuh must only be included when use_rs_w4a8=1");
...
__global__ __launch_bounds__(TuningConfig::kNumThreads) void humming(
  ...
  constexpr uint32_t kLoadThreadRegisters =
      (TuningConfig::kNumMathThreads > 256 ||
       (TuningConfig::kNumCtasPerSm == 1 && ElementA::kBits != 16) ? 40 : 24);
  ...
  using Ctx = KernelContext<
      ...
      LayerConfig, ComputeConfig, TuningConfig>;
  // Flat 16-CTA raster over grouped expert tiles.
  using KernelScheduler = RsW4a8GroupedScheduler<Ctx>;
  using ProducerPipeline = ProducerPipeline<Ctx>;
  using ConsumerPipeline = ConsumerPipeline<Ctx>;
  using MainloopArithmeticState = MainloopArithmetic<Ctx>;
  using EpilogueArithmetic = EpilogueArithmetic<Ctx>;
  using MMA = Mma<Ctx, MainloopArithmeticState>;
  ...
  static_assert(Ctx::kUseWgmma && !Ctx::kUseUmma);
  ...
  // RsW4a8GroupedScheduler is an all-thread cooperative ctor (loads the
  // expert prefix table into smem + __syncthreads); it sits before the
  // load/math split, exactly where the generic Scheduler(ctx) did, so all
  // threads reach it.
  auto scheduler = KernelScheduler(
      smem, shape_m, use_int64_expert_layout, expert_layout_ptr,
      param_to_ptr(C), tensor_map_buffer);
  ...
  if (ctx.is_load_thread()) {
    ...
    while (scheduler.get_next_block()) {
      ...
      for (uint32_t stage_id = 1; stage_id < MAX(kNumStages - 1, 2); stage_id++) {
        const bool pred = stage_id < slice_iters;
        producer.load_stage(stage_id, pred);
      };
      constexpr uint32_t num_slice_iters = ProblemShape::K / BlockShape::K;
      auto produce_stage = [&](auto stage, uint32_t slice_iter) {
        ...
        if constexpr (kNumStages == 2) {
          const bool pred = remaining_iters > kNumStages;
          producer.load_stage(stage_id, pred);
        } else {
          constexpr uint32_t target_stage = (stage_id + kNumStages - 1) % kNumStages;
          const bool pred = remaining_iters >= kNumStages;
          producer.load_stage(target_stage, pred);
        }
      };
    ...
    constexpr uint32_t kEstimatedMathThreadRegisters =
        MIN(232, MAX(128, kAccumulatorRegistersPerThread * 2 + 96));
    ...
    auto mainloop_arith = MainloopArithmeticState();
    ...
    while (scheduler.get_next_block()) {
      debug_kernel_timeout_check(debug_start_clock);
      if constexpr (MMA::kUseSharedASPromotion)
        mma.set_m_scale_offset(scheduler.m_offset);
      mma.zero_accum();

      epilogue.seek(
          scheduler.expert_id, scheduler.m_block_id, scheduler.n_block_id,
          scheduler.current_shape_m, scheduler.m_offset);
      epilogue.set_streamk_state(
          scheduler.slice_count, scheduler.slice_id,
          scheduler.locks_offset);
